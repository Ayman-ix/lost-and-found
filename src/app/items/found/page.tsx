...
function FoundItemsContent() {
  ...
  const loadFoundItems = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/items/found?${params.toString()}`);
      if (res.ok) {
              ...
              <div>
                <div className="relative mb-3 flex h-40 w-full items-center justify-center rounded-xl bg-zinc-100 overflow-hidden">
                  {item.images?.[0]?.image_url ? (
                    ...
                    <CheckCircle className="h-10 w-10 text-zinc-400 group-hover:text-emerald-500 transition" />
                  )}
                  <span className="absolute top-2.5 left-2.5 rounded-md bg-emerald-600 px-2 py-0.5 text-[10px] font-bold uppercase text-white">
                    FOUND
                  </span>
...
export default function FoundItemsPage() {
  return (
    <Suspense fallback={<div className="p-10 text-center text-zinc-500">Loading found directory...</div>}>
      <FoundItemsContent />
    </Suspense>
  );
}
